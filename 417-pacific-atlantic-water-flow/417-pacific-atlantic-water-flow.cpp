class Solution {
    void bfs(vector<vector<int>>& heights, vector<vector<int>>& matrix, int ocean, int i, int j) {
        ...
        if (i > 0 && (matrix[i - 1][j] & ocean) != ocean && heights[i - 1][j] >= heights[i][j])
            bfs(heights, matrix, ocean, i - 1, j);
        if (i < matrix.size() - 1 && (matrix[i + 1][j] & ocean) != ocean && heights[i + 1][j] >= heights[i][j])
            bfs(heights, matrix, ocean, i + 1, j);
        if (j > 0 && (matrix[i][j - 1] & ocean) != ocean && heights[i][j - 1] >= heights[i][j])
            bfs(heights, matrix, ocean, i, j - 1);
        if (j < matrix[0].size() - 1 && (matrix[i][j + 1] & ocean) != ocean && heights[i][j + 1] >= heights[i][j])
            bfs(heights, matrix, ocean, i, j + 1);
    }
public:
